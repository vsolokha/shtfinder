...
class Shot:
    def __init__(self, number, shtpath, unpack_method="exe"):
        self.number  = number
    ...
    def read(self):
        if self.unpack_method == "exe":
            ...
            if os.name == 'nt':
                os.system(r'./res/test.exe "{0}" "{1}"'.format(self.shtpath + filename_sht, filename))
            else:
                os.system(r'WINEDEBUG=fixme-all LANG=ru_RU.UTF-8 wine ./res/test.exe "{0}" "{1}"'.format(self.shtpath + filename_sht, filename))
            try:
    ...
    def print_names(self):
        ...
        for name in self.names:
            print(i, name)#.decode('cp1251').encode('utf8'))
            i += 1
...
def get_distance(baseshot_number, testshot_range, columns, shtpath, method="dtw", unpack_method='exe'):
    baseshot = Shot(baseshot_number, shtpath, unpack_method)
    number   = [] 
    result   = []
    for testshot_number in tqdm(range(testshot_range[0], testshot_range[1]+1)):
        try: 
            testshot = Shot(testshot_number, shtpath, unpack_method)
...
def get_wsorted(number, res, weight, islong=False):
    ...
    if islong:
        ...
        return np.c_[number[np.argsort(dist_flat)], dist_flat[np.argsort(dist_flat)]]
